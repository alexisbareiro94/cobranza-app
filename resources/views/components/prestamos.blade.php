    ...
    <div class="bg-gray-100 rounded-lg shadow p-3 hover:shadow-md transition">
                ...
                <div class="flex" >
                    ...
                    <select class="ml-3 text-sm px-2 py-1 border border-gray-300 rounded-md"
                        ...
                        @foreach ($prestamo->pagos as $pago)                            
                            <option value="" disabled>{{ format_fecha($pago->vencimiento)}} ● <span>{{ set_estado_pago($pago->estado) }}</option>
                        @endforeach
                    </select>                    
                </div>
        ...
        <div class="mt-3 flex justify-between items-center relative">
            <span id="prueba" @class([
                ...
                'bg-yellow-200 text-yellow-700' => $prestamo->proximo_pago->estado == 'pendiente',
                'bg-orange-300 text-orange-700' => $prestamo->proximo_pago->estado == 'parcial',
                'bg-red-200 text-red-700' => $prestamo->proximo_pago->estado == 'no_pagado',
                'bg-green-200 text-green-700' => $prestamo->proximo_pago->estado == 'pagado',
                ])
            >
